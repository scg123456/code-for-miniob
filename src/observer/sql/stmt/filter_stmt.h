...
public:
  ...
  FilterStmt(std::vector<std::unique_ptr<Expression>> expressions)
      : filter_expressions_(std::move(expressions)) {}

public:
  std::vector<std::unique_ptr<Expression>> &filter_expressions() { return filter_expressions_; }

public:
  static RC create(Db *db, ExpressionBinder &expression_binder, std::vector<std::unique_ptr<Expression>> &&predicate, FilterStmt *&stmt);
